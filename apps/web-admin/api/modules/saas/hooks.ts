...
export function useDeletePlan() {
  ...
  return useMutation({
    mutationFn: (id: string) =>
      deleteApi<ApiResponse<{ ok: boolean }>>(`/api/v1/admin/plans/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "plans"] });
      queryClient.invalidateQueries({ queryKey: ["admin", "overview"] });
    },
...
export function useDeleteTenant() {
  ...
  return useMutation({
    mutationFn: (id: string) =>
      deleteApi<ApiResponse<{ ok: boolean }>>(`/api/v1/admin/tenants/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "tenants"] });
      queryClient.invalidateQueries({ queryKey: ["admin", "subscriptions"] });
      queryClient.invalidateQueries({ queryKey: ["admin", "overview"] });
...
export function useAdminSubscriptions(options?: { enabled?: boolean }) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      getApi<ApiResponse<Subscription[]>>("/api/v1/admin/subscriptions"),
    enabled: (options?.enabled ?? true) && Boolean(accessToken),
  });
}
...
export function useUpdateSubscription() {
  ...
  return useMutation({
    mutationFn: ({ id, body }: { id: string; body: SubscriptionUpdateInput }) =>
      putApi<ApiResponse<Subscription>>(
