          ...
          <Card className="border-default bg-surface">
            <div className="mb-4 flex flex-wrap items-center gap-2">
              ...
              <Tag color={settings?.configured ? 'green' : 'default'}>
                {settings?.configured ? `${settings.mode} mode` : 'Not configured'}
              </Tag>
              {settings?.secret_key_set && <Tag color="blue">Secret key set</Tag>}
              {settings?.webhook_secret_set && <Tag color="purple">Webhook secret set</Tag>}
            </div>
            ...
            <Form form={form} layout="vertical">
              <Form.Item name="secret_key" label="Secret key" extra="Leave blank to keep existing">
                <Input.Password placeholder="sk_test_..." />
              </Form.Item>
              <Form.Item name="publishable_key" label="Publishable key">
                <Input placeholder="pk_test_..." />
              </Form.Item>
              <Form.Item name="webhook_secret" label="Webhook signing secret" extra="Leave blank to keep existing">
