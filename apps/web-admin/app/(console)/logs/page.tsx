...
export default function AdminLogsPage() {
  ...
  const formTable = useFormTable<Log, LogFilters>({
    ...
    filterFn: (filter, row) => {
      if (filter.action && !row.action.toLowerCase().includes(filter.action.toLowerCase())) return false
      if (filter.resource && !row.resource.toLowerCase().includes(filter.resource.toLowerCase())) return false
      return true
    },
    columns: [
