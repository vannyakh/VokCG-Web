...
import { EntityCell, FormTablePage } from '@vokcg/ui/admin'
...
import { formatAdminDate, formatCurrency } from '@vokcg/ui/lib/admin-format'
...
export default function AdminTenantsPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      message.error(editing ? 'Failed to update tenant' : 'Failed to create tenant')
    }
  }
  ...
  const formTable = useFormTable<Tenant, TenantFilters>({
    ...
    onRefresh: () => refetch(),
    formSchema: [
      { name: 'name', label: 'Organization', placeholder: 'Search name or slug' },
      {
        name: 'status',
        label: 'Status',
        type: 'select',
    ...
    filterFn: (filter, row) => {
      if (filter.name) {
        ...
        if (!row.name.toLowerCase().includes(q) && !row.slug.toLowerCase().includes(q)) {
          return false
        }
      }
  ...
  return (
    <FormTablePage
      ...
      stats={[
        ...
        { label: 'Combined MRR', value: totalMrr, icon: DollarSign, prefix: '$' },
        {
          label: 'Total members',
          value: totalMembers,
          icon: Users,
      ...
      >
        <Form form={form} layout="vertical" className="mt-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Form.Item name="members_count" label="Member seats" rules={[{ required: true }]}>
