...
export function AdminSidebar({
  ...
  const { sidebarWidth, setSidebarWidth, sidebarMiniMode, sidebarHoverExpand } =
    useAdminUiStore();
  ...
  const onDragStart = useCallback(
    (e: React.MouseEvent) => {
      ...
      const onMove = (ev: MouseEvent) =>
        setSidebarWidth(
          Math.max(
            ADMIN_SIDEBAR.widthMin,
            Math.min(ADMIN_SIDEBAR.widthMax, startW + ev.clientX - startX),
          ),
        );
      const onUp = () => {
  ...
  return (
    ...
    >
      <motion.aside
        ...
        style={{
          boxShadow: hovering ? "4px 0 24px rgba(0,0,0,0.12)" : undefined,
        }}
      >
        {/* ── Brand ──────────────────────────────────────────── */}
