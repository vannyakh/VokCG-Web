...
export function PlansPage() {
  ...
  const stats = useMemo(() => {
    ...
    const totalSubscribers = plans.reduce(
      (sum, p) => sum + Number(p.subscribers),
      0,
    );
    const avgPrice =
      plans.length > 0
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      message.error(
        editing ? "Failed to update plan" : "Failed to create plan",
      );
    }
  };
  ...
  const formTable = useFormTable<Plan, PlanFilters>({
    ...
    filterFn: (filter, row) => {
      if (
        ...
        return false;
      if (filter.active !== undefined && row.active !== filter.active)
        return false;
      return true;
    },
    columns: [
      ...
      columnHelper.display({
        ...
        cell: ({ row }) => (
          <span className="text-sm font-medium text-primary">
            {formatCurrency(Number(row.original.price), { prefix: "$" })}/
      ...
      columnHelper.accessor("active", {
        ...
        size: 90,
        cell: (info) => (
          <Switch checked={info.getValue()} size="small" disabled />
        ),
      }),
      columnHelper.display({
        id: "actions",
