...
export function buildEntitiesBarOption(
  stats: AdminOverview,
  isDark: boolean,
): EChartsCoreOption {
  ...
  return {
    ...
    xAxis: {
      ...
      axisLine: {
        lineStyle: { color: isDark ? "rgba(255,255,255,0.08)" : "#e2e8f0" },
      },
    },
    yAxis: {
      type: "value",
      minInterval: 1,
      splitLine: {
        lineStyle: { color: isDark ? "rgba(255,255,255,0.06)" : "#f1f5f9" },
      },
      axisLabel: baseTextStyle(isDark),
    },
    series: [
...
export function buildAuditActionOption(
  logs: AuditLog[],
  isDark: boolean,
): EChartsCoreOption {
...
export function buildAuditTimelineOption(
  logs: AuditLog[],
  isDark: boolean,
): EChartsCoreOption {
    ...
    series: [
      {
        ...
        axisLine: {
          lineStyle: {
            width: 10,
            color: [[1, isDark ? "rgba(255,255,255,0.08)" : "#e2e8f0"]],
          },
        },
        axisTick: { show: false },
