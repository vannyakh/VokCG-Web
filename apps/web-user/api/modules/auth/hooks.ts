...
export function useAuthSessions(enabled = true) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      getApi<ApiResponse<AuthSession[]>>(
...
export function useRevokeOtherSessions() {
  ...
  return useMutation({
    mutationFn: () =>
      postApi<ApiResponse<{ ok: boolean; revoked: number }>>(
...
export function useUserApiAccessLogs(apiKeyId?: string, page = 1) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      getApi<ApiResponse<PaginatedApiAccessLogs>>(
        "/api/v1/auth/me/api-access-logs",
        {
          page,
          page_size: 15,
          api_key_id: apiKeyId,
