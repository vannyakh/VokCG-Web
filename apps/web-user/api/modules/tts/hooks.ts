...
import { deleteApi, getApi, postBlob, uploadFormData } from "@/api/request";
...
export function useVoiceClones(provider?: string, q?: string) {
  return useQuery({
    ...
    queryFn: () =>
      getApi<ApiResponse<{ clones: VoiceCloneProfile[] }>>(
...
export function useCreateVoiceClone() {
  ...
  return useMutation({
    mutationFn: (formData: FormData) =>
      uploadFormData<ApiResponse<{ clone: VoiceCloneProfile }>>(
        "/api/v1/tts/voice-clones",
        formData,
      ),
    onSuccess: () => {
  ...
  return useMutation({
    mutationFn: (cloneId: string) =>
      deleteApi<ApiResponse<{ ok: boolean }>>(
        `/api/v1/tts/voice-clones/${cloneId}`,
      ),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: TTS_CLONES_KEY });
      queryClient.invalidateQueries({ queryKey: TTS_SERVERS_KEY });
