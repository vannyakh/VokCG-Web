...
import { ArrowRight, Calendar, CreditCard, HardDrive, Package, Users, Video } from 'lucide-react'
import type { ReactNode } from 'react'
import Link from 'next/link'
import { useLocale } from '@vokcg/i18n'
import { useTasks } from '@vokcg/api'
import { useWorkspace } from '@vokcg/api'
import { useWorkspacePlans } from '@vokcg/api'
import { USER_ROUTES } from '@vokcg/constants'
import { formatAdminDate, formatCurrency } from '@vokcg/ui'
import { parsePlanLimits, pickUpgradePlan } from './lib/workspace-billing-utils'
...
function StatusPill({ children, tone = 'default' }: { children: ReactNode; tone?: 'default' | 'accent' | 'muted' }) {
  ...
  return (
    <span className={`rounded-full px-3 py-1 text-[11px] font-bold uppercase tracking-wide ${toneClass}`}>
      {children}
    </span>
  )
}
  ...
  return (
    <Link
      ...
      className={[
        ...
        variant === 'primary' ? 'billing-primary-btn' : 'billing-primary-btn--ghost',
      ].join(' ')}
    >
      {icon}
      <span>{children}</span>
...
export function BillingPage() {
  ...
  const statusLabel = workspace.subscription_status?.replace('_', ' ') ?? t('billing.statusActive')
  ...
  return (
    <BillingShell
      ...
      extra={
        canManageBilling ? (
          <RoundedLinkButton href={USER_ROUTES.billingPlans} icon={<Package size={16} />}>
            {t('billing.viewPlans')}
          </RoundedLinkButton>
        ) : undefined
      }
      ...
      <div className="flex flex-col gap-8">
        {upgradePlan && canManageBilling && (
          <section className="billing-upgrade-banner flex flex-col gap-5 rounded-[20px] px-6 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-7 sm:py-6">
            <div className="min-w-0 space-y-1">
              <p className="text-base font-bold text-primary">{t('billing.upgradeBannerTitle')}</p>
              <p className="text-[15px] text-muted">
                {t('billing.upgradeBannerHint', { plan: upgradePlan.name })}
              </p>
            </div>
            <RoundedLinkButton href={USER_ROUTES.billingPlans} icon={<ArrowRight size={16} />}>
        ...
        <BillingList title={t('billing.subscription')}>
            ...
            value={
              <div className="flex flex-col items-end gap-2.5">
                <p className="text-[28px] font-extrabold leading-none tabular-nums tracking-tight text-primary">
                  ...
                  <span className="ml-1 text-base font-medium text-muted">/{plan?.interval ?? 'month'}</span>
                </p>
                <div className="flex flex-wrap justify-end gap-2">
                  <StatusPill tone="accent">{statusLabel}</StatusPill>
                  {isFreePlan && <StatusPill tone="muted">{t('billing.freeTier')}</StatusPill>}
                </div>
              </div>
            }
          />
          <BillingListRow
            ...
            hint={workspace.name}
            value={<span className="text-[15px] font-semibold text-primary">{renewalLabel}</span>}
          />
          <BillingListRow
            icon={<Users size={17} />}
        ...
        <BillingList title={t('billing.planCredits')} description={t('billing.planCreditsHint')}>
          <BillingCreditStat
            icon={Video}
