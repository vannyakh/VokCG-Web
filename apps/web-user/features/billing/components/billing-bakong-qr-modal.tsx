...
export function BillingBakongQrModal({
  open,
  plan,
  qrCode,
  amount,
  ...
  return (
    <BillingModalShell
      open={visible}
      onClose={onClose}
      ariaLabelledBy="billing-bakong-title"
    >
      {plan && qrCode && (
        <>
          <div className="relative border-b border-white/[0.08] bg-[#1a1f2e] px-6 pb-5 pt-6">
            <button
              type="button"
              className="absolute right-5 top-5 flex h-9 w-9 items-center justify-center rounded-full border border-white/[0.08] bg-white/[0.06] text-white/55"
              onClick={onClose}
              aria-label="Close"
            >
              <X size={16} />
            </button>
            <p className="mb-1.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#ffb4b4]/80">
              KHQR
            </p>
            <h2
              id="billing-bakong-title"
            ...
            <div className="flex items-end gap-1.5">
              ...
              <span className="text-[40px] font-semibold leading-none tracking-tight text-white">
                {formatCheckoutAmount(displayAmount)}
              </span>
              <span className="pb-1 text-sm font-medium text-white/45">
                USD
              </span>
            </div>
          </div>
