...
export function BillingPlanCard({
  ...
  return (
    <article
      ...
      className={[
        "flex flex-col gap-5 px-5 py-6 sm:px-7 sm:py-7 lg:flex-row lg:items-center lg:gap-8",
        isCurrent
          ? "bg-accent/[0.06]"
          : isRecommended
            ? "bg-accent/[0.03]"
    ...
    >
      ...
      <div className="flex shrink-0 flex-col items-stretch gap-4 sm:flex-row sm:items-center sm:gap-6 lg:flex-col lg:items-end lg:gap-3">
        ...
        <button
          ...
          onClick={onSelect}
          className={[
            "inline-flex h-11 min-w-[148px] items-center justify-center gap-2 rounded-full px-6 text-sm font-semibold",
            "disabled:cursor-not-allowed disabled:opacity-60",
            isCurrent ? "billing-primary-btn--outline" : "billing-primary-btn",
          ].join(" ")}
        >
          {loading && !isCurrent ? (
