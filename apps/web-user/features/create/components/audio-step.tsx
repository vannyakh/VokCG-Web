...
function AudioSettingsSection() {
  ...
  const {
    ...
    isError: voicesError,
  } = useTtsVoices(audio.ttsServer, locale, audio.voiceName);
  ...
  const selectedServer = useMemo(
    () => availableServers.find((server) => server.id === audio.ttsServer),
    [availableServers, audio.ttsServer],
  );
  ...
  useEffect(() => {
    ...
    if (availableServers.some((server) => server.id === audio.ttsServer))
      return;
    const nextServer =
  ...
  useEffect(() => {
    ...
    if (nextVoice && nextVoice !== audio.voiceName)
  ...
  const handleCloneSelect = (clone: VoiceCloneProfile) => {
    const server = CLONE_PROVIDERS.find(
      (p) => p.id === clone.provider,
    )?.serverId;
  ...
  const handlePlayVoice = async () => {
    ...
    setPreviewError("");
    const text =
      previewText.trim() || subject.trim() || script.trim() || "Voice Example";
    try {
      const blob = await voicePreview.mutateAsync({
        ...
        style_prompt: showMimoStyle ? audio.stylePrompt : undefined,
      });
      if (previewUrl) URL.revokeObjectURL(previewUrl);
  ...
  return (
    <div className="flex flex-col gap-4">
      <FieldMenuSelect
        label={t("ttsStudio.server")}
        options={serverOptions}
        value={audio.ttsServer}
        onChange={(v) => patch({ ttsServer: v })}
        disabled={serversLoading}
        placeholder={t("ttsStudio.serverPlaceholder")}
      />
      ...
      {availableServers.length <= 1 && !serversLoading && (
        <Alert
          type="info"
          showIcon
          description="Add provider API keys in config.toml or .env to enable speech synthesis."
        />
      )}

      {showClonePanel && (
        <VoiceClonePanel compact onCloneSelect={handleCloneSelect} />
      )}
          ...
          {voicesLoading ? (
            <div className="flex items-center gap-2 py-2">
              ...
              <span className="text-xs text-muted">
                {t("ttsStudio.loadingVoices")}
              </span>
            </div>
          ) : voicesError || voices.length === 0 ? (
            <div className="rounded-lg border border-error bg-app-error px-3 py-2.5">
              <span className="text-xs font-semibold text-error">
                {t("ttsStudio.noVoices")}
              </span>
            </div>
          ) : (
            <MenuSelect
        ...
        <>
          <div className="flex flex-col gap-1.5">
            ...
            <Input.TextArea
              ...
              rows={2}
              placeholder="Text to synthesize for preview…"
              className="text-sm"
            />
          </div>
          <div>
            <Button
              ...
              disabled={
                voicePreview.isPending || !audio.voiceName || !selectedServer
              }
              icon={
      ...
      <RangeField
        ...
        max={5.0}
        step={0.1}
        onChange={(v) => patch({ voiceVolume: v })}
      />
      <RangeField
      ...
      {audio.bgmType === "custom" && (
        <div className="flex flex-col gap-1.5">
          ...
          <Input
            value={audio.bgmFile}
            onChange={(e) => patch({ bgmFile: e.target.value })}
...
export function AudioStep() {
  return (
    <StepSection
      title="Voice & music"
      description="Select a TTS provider, voice clone, speech rate, and background music."
    >
