...
const FORM_COLOR_PRESETS = [
  {
    label: "Common",
    colors: ["#FFFFFF", "#000000", "#FACC15", "#38BDF8", "#F472B6", "#4ADE80"],
  },
];
...
export function Toggle({
  ...
  return (
    ...
    >
      <div
        className={[
          "relative h-5 w-[34px] shrink-0 rounded-full border transition-[background,border-color] duration-200",
          checked ? "border-accent bg-accent" : "border-default bg-subtle",
        ].join(" ")}
      >
        <div
          className={[
            "absolute top-[3px] h-3 w-3 rounded-full bg-white shadow-[0_1px_3px_rgba(0,0,0,0.25)] transition-[left] duration-200",
            checked ? "left-[17px]" : "left-[3px]",
          ].join(" ")}
        />
      </div>
      <span className="select-none text-sm font-semibold text-primary">
        {label}
      </span>
    </button>
  );
...
export function CreateFieldLabel({ children }: { children: ReactNode }) {
  return (
    <label className="text-[13px] font-bold text-primary">{children}</label>
  );
}
...
export function CreateCard({
  ...
  return (
    <div
      className={[
        "overflow-hidden rounded-2xl border border-subtle bg-surface",
        "shadow-[var(--shadow-sm)] ring-1 ring-[color-mix(in_srgb,var(--color-primary)_5%,transparent)]",
        className,
      ].join(" ")}
    >
      {children}
    ...
    <div className="rounded-xl border border-subtle bg-subtle/30">
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            <div className="flex flex-col gap-4 border-t border-subtle px-3.5 pb-4 pt-3">
              {children}
            </div>
          </motion.div>
        )}
