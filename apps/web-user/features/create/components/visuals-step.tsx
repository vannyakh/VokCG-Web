...
import {
  useVideoAppSettings,
  useDeleteVideoMaterial,
  useUploadVideoMaterial,
  VIDEO_ENCODER_HELP,
} from "@/api";
...
const VIDEO_CODECS = [
  ...
  { value: "h264_mf", label: "Windows MediaFoundation (h264_mf)" },
  {
    ...
    label: "macOS VideoToolbox (h264_videotoolbox)",
  },
] as const;
...
function VideoSettingsSection() {
  ...
  const handleDeleteMaterial = async (name: string) => {
    ...
    try {
      ...
      patch({
        materials: visuals.materials.filter((m) => m.url !== name),
        uploadedMaterialNames: visuals.uploadedMaterialNames.filter(
          (n) => n !== name,
        ),
      });
    } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {visuals.source === "local" && (
        <div className="flex flex-col gap-1.5">
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-lg border border-dashed border-default bg-subtle px-4 py-2.5 text-sm font-semibold text-secondary hover:border-accent hover:text-primary disabled:opacity-50"
          >
      ...
      <div className="border-t border-default pt-3">
        <button
          type="button"
          className="flex w-full items-center justify-between py-1 hover:opacity-85"
              ...
              <div className="pt-4">
                <div className="flex flex-col gap-1.5">
                  <div className="mb-1 flex items-center gap-1.5">
                    ...
                    <span
                      className="cursor-help select-none text-xs text-muted"
                      title={VIDEO_ENCODER_HELP}
                    >
                      ?
                    </span>
                  ...
                  <MenuSelect
                    ...
                    value={videoCodec}
                    onChange={(v) => saveSettings({ video_codec: v })}
...
export function VisualsStep() {
  return (
    <StepSection
      ...
      description="Pick footage sources, aspect ratio, clip duration, and transitions."
    >
