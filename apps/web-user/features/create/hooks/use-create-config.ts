...
import {
  canRunContentGeneration,
  CONTENT_GENERATION_DEPS,
...
function useContentGeneration(
  ...
  return useMemo(
    () => ({
      ...
      canRun: (action: ContentGenerationAction) =>
        canRunContentGeneration(action, config),
    }),
    [config],
  );
}
...
export function useCreateConfig<S extends CreateConfigSection>(
  section: S,
): {
  config: CreateStudioConfig;
...
export function useCreateConfig<S extends CreateConfigSection | undefined>(
  section?: S,
) {
