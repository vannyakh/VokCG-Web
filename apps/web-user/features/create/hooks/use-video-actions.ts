...
import {
  getCreateStudioConfig,
  useCreateStudioStore,
  usePreviewOutputStore,
  useVideoStore,
} from "@/store";
...
export function useVideoActions() {
  ...
  const generateTermsMutation = useMutation({
    mutationFn: () => {
      const config = getCreateStudioConfig();
      return postApi<TermsResponse>(
        "/api/v1/terms",
        configToTermsPayload(config),
      );
    },
    onSuccess: (payload) =>
      patchConfig("content", {
        terms: (payload.data.video_terms || []).join(", "),
      }),
  });
  ...
  const createVideoMutation = useMutation({
    mutationFn: () => {
      const config = getCreateStudioConfig();
      return postApi<TaskCreateResponse>(
        "/api/v1/videos",
        configToVideoPayload(config),
      );
    },
    onSuccess: () => {
