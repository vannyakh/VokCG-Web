...
export function mergeCreateConfig(
  partial?: DeepPartialCreateConfig | null,
): CreateStudioConfig {
...
export function configToScriptPayload(
  config: CreateStudioConfig,
  extras?: ScriptWriterPayloadExtras,
) {
...
export function configToTermsPayload(
  config: CreateStudioConfig,
  amount = 5,
  purpose: TermsPayloadPurpose = "search",
) {
  const { content } = config;
  return {
    video_subject: content.subject,
...
export function canRunContentGeneration(
  ...
  switch (action) {
    case "generateScript":
      return Boolean(content.subject.trim());
    case "generateTerms":
      return Boolean(content.script.trim());
    default:
      return false;
  }
}
...
export const CONTENT_GENERATION_DEPS: Record<
  ContentGenerationAction,
  {
    requires: readonly (keyof CreateStudioConfig["content"])[];
    provides: readonly (keyof CreateStudioConfig["content"])[];
  }
> = {
  generateScript: {
    requires: [
...
export function validateCreateFlowStep(
  ...
  config: CreateStudioConfig,
): CreateFlowStepValidation {
  switch (stepId) {
    case "content":
      return validateContent(config);
    case "visuals":
      return validateVisuals(config);
    case "audio":
      return validateAudio(config);
    case "subtitles":
      return validateSubtitles(config);
    case "review": {
      const checks = [
        "content",
        "visuals",
...
function validateVisuals(config: CreateStudioConfig): CreateFlowStepValidation {
  ...
  if (source === "local" && materials.length === 0)
    return { valid: false, messageKey: "create.validation.visualsRequired" };
  if (count < 1)
    return { valid: false, messageKey: "create.validation.countRequired" };
  if (clipDuration < 1)
    return {
      valid: false,
      messageKey: "create.validation.clipDurationRequired",
    };
  return { valid: true, messageKey: null };
}
