...
export function PublishPage() {
  ...
  return (
    <ExploreShell
      ...
      extra={<ExploreDisabledButton>{t('explore.addSchedule')}</ExploreDisabledButton>}
    >
      <div className="flex flex-col gap-5">
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span
                    className={[
                      ...
                      platform.connected ? 'bg-emerald-500/10 text-emerald-600' : 'bg-subtle text-muted',
                    ].join(' ')}
                  >
                    {platform.connected ? t('explore.connected') : t('explore.connect')}
                  </span>
                </div>
                <p className="mt-3 text-[14px] font-semibold text-primary">{platform.name}</p>
                <div className="mt-3">
        ...
        <section className="space-y-3">
          ...
          <div className="overflow-hidden rounded-2xl border border-divider bg-surface" style={{ boxShadow: 'var(--shadow-sm)' }}>
            {MOCK_SCHEDULE.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-2 px-6 py-12 text-center">
                <CalendarClock size={28} className="text-muted" />
                <p className="text-[14px] font-semibold text-primary">{t('explore.publish.scheduleEmpty')}</p>
                <p className="max-w-md text-[13px] text-muted">{t('explore.publish.scheduleEmptyHint')}</p>
              </div>
            ) : (
