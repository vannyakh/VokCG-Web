    ...
    <div className="overflow-hidden rounded-xl border border-default bg-surface shadow-md transition-shadow hover:shadow-lg">
      ...
      <div className="grid grid-cols-1 gap-3 p-4 md:grid-cols-2">
        <FieldSelect
          ...
          options={SCRIPT_PLATFORMS.map((item) => ({ value: item, label: t(`scriptWriter.platform.${item}`) }))}
        />
        <FieldSelect
          label={t('scriptWriter.targetLength')}
          value={length}
          onChange={(v) => onLengthChange(v as ScriptLength)}
          options={SCRIPT_LENGTHS.map((item) => ({ value: item, label: t(`scriptWriter.length.${item}`) }))}
        />
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-default bg-subtle/30 px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            type="primary"
            size="middle"
            icon={busy ? <Spin size="small" className="text-white" /> : <Sparkles size={13} />}
            onClick={onGenerate}
...
function FieldSelect({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label className="text-[10px] font-semibold uppercase tracking-wide text-muted">{label}</label>
      <Select size="small" value={value} onChange={onChange} options={options} className="w-full" />
    </div>
  )
}
