      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        <div className="border-b border-default px-3.5 py-2.5">
          ...
          <div className="grid grid-cols-2 gap-1.5">
            <StatBox
              value={hasScript ? String(wordCount) : "—"}
              label={t("scriptWriter.words")}
            />
            <StatBox
              value={hasScript ? duration : "—"}
              label={t("scriptWriter.estDuration")}
            />
            <StatBox
              value={hasScript ? String(gradeLevel) : "—"}
              label={t("scriptWriter.gradeLevel")}
            />
            <StatBox
              value={hasScript ? String(clarityScore) : "—"}
              label={t("scriptWriter.clarityScore")}
            />
          </div>
        </div>
        {hasScript && (
          <div className="border-b border-default px-3.5 py-2.5">
            ...
            <ScoreRow
              label={t("scriptWriter.engagement")}
              value={quality.engagement}
              color="bg-emerald-500"
            />
            <ScoreRow
              label={t("scriptWriter.clarity")}
              value={quality.clarity}
              color="bg-teal-500"
            />
            <ScoreRow
              label={t("scriptWriter.seoFit")}
              value={quality.seo}
              color="bg-amber-500"
            />
            <ScoreRow
              label={t("scriptWriter.hookStrength")}
              value={quality.hookStrength}
              color="bg-emerald-500"
            />
          </div>
        )}
        <div className="border-b border-default px-3.5 py-2.5">
          ...
          <div className="space-y-0.5">
            <ActionRow
              icon={RefreshCw}
              tone="purple"
              label={t("scriptWriter.actionRewrite")}
              onClick={() => onAiAction("rewrite")}
            />
            <ActionRow
              icon={ArrowUp}
              tone="teal"
              label={t("scriptWriter.actionShorter")}
              onClick={() => onAiAction("shorter")}
            />
            <ActionRow
              icon={List}
              tone="green"
              label={t("scriptWriter.actionChapters")}
              onClick={() => onAiAction("chapters")}
            />
            <ActionRow
              icon={Hash}
              tone="amber"
              label={t("scriptWriter.actionHashtags")}
              onClick={() => onAiAction("hashtags")}
            />
            <ActionRow
              icon={Sparkles}
              tone="coral"
              label={t("scriptWriter.actionHook")}
              onClick={() => onAiAction("hook")}
            />
            <ActionRow
              icon={Languages}
              tone="purple"
              label={t("scriptWriter.actionTranslate")}
...
function ScoreRow({
  ...
  return (
    <div className="mb-1.5 flex items-center gap-2">
      <span className="w-[68px] shrink-0 text-[11px] text-secondary">
        {label}
      </span>
      ...
      <span className="w-5 text-right text-[11px] font-medium text-secondary">
        {value}
      </span>
    </div>
  );
