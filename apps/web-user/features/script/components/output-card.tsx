  ...
  return (
    <div className="overflow-hidden rounded-xl border border-default bg-surface">
      <div className="flex flex-wrap items-center gap-2 border-b border-default px-3.5 py-2.5">
        ...
        <span className="text-xs font-medium text-secondary">{t('scriptWriter.generatedScript')}</span>
        ...
        <div className="ml-auto flex flex-wrap gap-1.5">
          ...
          <Button size="small" type="primary" icon={<Mic size={12} />} onClick={onSendToTts}>
        ...
        {sections.map((section) => (
          <div key={section.id} className="border-b border-default py-3 last:border-b-0">
            ...
            <div
              ...
              onInput={(e) => onSectionChange(section.id, e.currentTarget.textContent ?? '')}
          ...
          <div className="grid gap-2 md:grid-cols-3">
            {variations.map((variation) => (
              ...
              >
                ...
                <p className="mt-1 text-xs leading-relaxed text-secondary">{variation.text}</p>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="flex gap-4 border-t border-default bg-subtle px-3.5 py-2">
        ...
        <StatInline value={String(charCount)} label={t('scriptWriter.characters')} />
        <StatInline value={String(sections.length)} label={t('scriptWriter.sections')} />
      </div>
    </div>
  )
}
