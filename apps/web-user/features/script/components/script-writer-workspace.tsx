...
import {
  ...
  parseScriptSections, platformForTemplate, saveScriptHistory,
  SCRIPT_TEMPLATES, sectionNamesForTemplate, sectionsToScript,
...
export function ScriptWriterWorkspace() {
  ...
  const config = useMemo((): CreateStudioConfig => {
    ...
    return {
      ...
      content: { ...CREATE_CONFIG_DEFAULTS.content, subject, script, terms: keywordListToString(keywords), language, paragraphCount: paragraphsForLength(length), scriptPrompt },
    }
  }, [audience, keywords, language, length, platform, script, template, tone, topic])
    ...
    <>
      <div className="flex h-full min-h-0 overflow-hidden bg-canvas">
        <TemplatePanel
          ...
          history={history}
          onHistorySelect={(entry) => { setTemplateId(entry.templateId); setTopic(entry.title) }}
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <TopBar
            ...
            credits={credits}
            onTabChange={(next) => { if (next === 'tts') handleSendToTts(); else setTab(next) }}
            ...
            <div className="min-w-0 flex-1 overflow-y-auto p-4 sm:p-4">
              {tab === 'versions' && (
                <VersionsPanel
                  t={t}
                  versions={versions}
                  onRestore={(version) => applyScript(version.script, sectionNames)}
                />
              )}
              ...
              {tab === 'generate' && (
                <div className="flex flex-col gap-3.5">
                  <BriefCard
                    ...
                    onRemoveKeyword={(keyword) => setKeywords((prev) => prev.filter((item) => item !== keyword))}
                    onKeywordDraftChange={setKeywordDraft}
                    onStartAddKeyword={() => setAddingKeyword(true)}
                    onCancelAddKeyword={() => { setAddingKeyword(false); setKeywordDraft('') }}
                    onConfirmAddKeyword={() => { addKeyword(keywordDraft); setKeywordDraft(''); setAddingKeyword(false) }}
                    onAiKeywords={() => generateTermsMutation.mutate('keywords')}
                  ...
                  {hasScript && (
                    <OutputCard
                      ...
                      onPreviewSection={(section) => handleSendToTts(section.text)}
