  ...
  return (
    <aside className="hidden w-[224px] shrink-0 flex-col border-r border-default bg-surface lg:flex">
      <div className="px-3.5 py-3.5">
        <h2 className="mb-2.5 text-[13px] font-semibold text-primary">
          {t("scriptWriter.templates")}
        </h2>
        <Input
          variant="borderless"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        {history.length === 0 ? (
          <p className="px-3.5 pb-3 text-xs text-muted">
            {t("scriptWriter.noHistory")}
          </p>
        ) : (
          history.map((entry) => (
            <div
              key={entry.id}
              className="group flex w-full items-center justify-between gap-1.5 px-3.5 py-1 hover:bg-subtle"
            >
              <button
                type="button"
              ...
              >
                ...
                <span className="text-[10px] text-muted shrink-0">
                  {relativeHistoryTime(entry.createdAt)}
                </span>
              </button>
              <button
                type="button"
