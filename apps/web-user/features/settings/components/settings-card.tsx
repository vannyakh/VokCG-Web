...
export function SettingsCard({
  ...
  return (
    <section
      className={[
        "overflow-hidden rounded-[20px] border border-default bg-surface",
...
export function SettingsCardHeader({
  ...
  return (
    <div className="flex items-center gap-3.5 border-b border-divider px-5 py-5 sm:px-6">
      <div
        className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${TONE_CLASS[tone]}`}
      >
...
export function SettingsCardBody({
  ...
  className = "",
  flush = false,
}: SettingsCardBodyProps) {
  return (
    <div
      className={[
        flush ? "px-5 sm:px-6" : "px-5 py-5 sm:px-6 sm:py-6",
        className,
      ]
        .filter(Boolean)
        .join(" ")}
