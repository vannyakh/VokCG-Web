...
export function SettingsServiceApiTab() {
  ...
  const { data: logsData, isLoading: logsLoading } = useUserApiAccessLogs(
    logKeyFilter,
    logPage,
  );
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (!secret) {
        message.error(t("settings.serviceApi.createFailed"));
        return;
      }
      setCreateOpen(false);
      setKeyName("");
      ...
      message.error(
        error instanceof Error
          ? error.message
          : t("settings.serviceApi.createFailed"),
      );
    }
  };
  ...
  const handleRevoke = async (keyId: string) => {
    try {
      ...
      message.error(
        error instanceof Error
          ? error.message
          : t("settings.serviceApi.revokeFailed"),
      );
    }
  };
    ...
    <div className="flex max-w-3xl flex-col gap-6">
      <SettingsCard>
        <SettingsCardHeader
          icon={Link2}
          tone="blue"
          title={t("settings.serviceApi.connectionTitle")}
          subtitle={t("settings.serviceApi.hint")}
        />
        <SettingsCardBody>
          <SettingsKvRow
            label={t("settings.serviceApi.baseUrl")}
            value={
              <code className="truncate rounded-lg bg-subtle px-2.5 py-1 font-mono text-sm text-muted">
                {API_BASE_URL || window.location.origin}
              </code>
            }
          />
          <SettingsKvRow
            label={t("settings.serviceApi.authHeader")}
            value={
              <code className="truncate rounded-lg bg-subtle px-2.5 py-1 font-mono text-sm text-muted">
                X-Api-Key
              </code>
      ...
      <SettingsCard>
        <SettingsCardHeader
          ...
          extra={
            ...
            >
              {t("settings.serviceApi.generate")}
            </SettingsPrimaryButton>
          }
        />
          ...
          {isLoading ? (
            ...
            (apiKeys ?? []).map((key) => (
              <SettingsListRow
                ...
                action={
                  <Popconfirm
                    title={t("settings.serviceApi.revokeConfirm")}
              ...
              >
                ...
                <div className="mt-1 flex flex-wrap items-center gap-2">
                  <code className="rounded-md bg-subtle px-2 py-0.5 font-mono text-sm text-muted">
                    {key.prefix}***
                  </code>
                  {key.scopes.map((scope) => (
                    <Tag key={scope} className="m-0 text-[11px]">
                      {scope}
                    </Tag>
                  ))}
                </div>
                <p className="mt-1 text-sm text-muted">
                  {t("settings.serviceApi.createdAt")}{" "}
                  {formatDate(key.created_at)}
                  {key.last_used
                    ? ` · ${t("settings.serviceApi.lastUsed")} ${formatDate(key.last_used)}`
                    : ` · ${t("settings.serviceApi.neverUsed")}`}
                </p>
              </SettingsListRow>
            ))
          )}
      ...
      <SettingsCard>
        <SettingsCardHeader
          ...
          extra={
            <Select
              ...
              onChange={(value) => {
                setLogKeyFilter(value || undefined);
                setLogPage(1);
              }}
            />
          }
        ...
        <SettingsCardBody flush>
          {logsLoading ? (
            <p className="py-6 text-[15px] text-muted">
              {t("settings.serviceApi.logsLoading")}
            </p>
          ) : logs.length === 0 ? (
            <p className="py-6 text-[15px] text-muted">
              {t("settings.serviceApi.logsEmpty")}
            </p>
          ) : (
            logs.map((log) => (
              <SettingsListRow key={log.id}>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <code className="truncate text-sm text-secondary">
                    {log.path}
                  </code>
                </div>
                <p className="mt-1 text-sm text-muted">
                  {log.api_key_name ??
                    log.api_key_prefix ??
                    t("settings.serviceApi.unknownKey")}
                  {" · "}
        ...
        {logsTotal > 15 && (
          <div className="flex items-center justify-between gap-4 border-t border-divider px-5 py-4 sm:px-6">
            <span className="text-sm text-muted">
              {t("settings.serviceApi.logsTotal", { count: String(logsTotal) })}
            </span>
      ...
      >
        <p className="mb-3 text-sm text-muted">
          {t("settings.serviceApi.generateHint")}
        </p>
        <Input
          value={keyName}
          onChange={(e) => setKeyName(e.target.value)}
          placeholder={t("settings.serviceApi.namePlaceholder")}
          maxLength={120}
      ...
      <Modal
        ...
        footer={[
          ...
          >
            {t("settings.serviceApi.secretDone")}
          </SettingsPrimaryButton>,
        ]}
        destroyOnHidden
      >
        <p className="mb-3 text-sm text-muted">
          {t("settings.serviceApi.secretHint")}
        </p>
        <div className="flex items-center gap-2 rounded-xl bg-subtle/60 p-3">
          <code className="min-w-0 flex-1 break-all text-sm text-primary">
            {createdSecret}
          </code>
          <button
            type="button"
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-secondary"
            onClick={() => createdSecret && void handleCopy(createdSecret)}
          >
            <Copy size={16} />
          </button>
        </div>
