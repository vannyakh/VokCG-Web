  ...
  return (
    <div className="overflow-hidden rounded-xl border border-default bg-surface">
      <div className="flex items-center gap-2 border-b border-default px-3.5 py-2.5">
        <span className="text-xs font-medium text-secondary">{t('ttsStudio.preview')}</span>
      ...
      <div className="flex items-center gap-2.5 px-3.5 py-3">
        ...
        <div className="ml-auto flex gap-1.5">
          <Button size="small" disabled icon={<Scissors size={13} />} className="text-xs">
            {t('ttsStudio.trim')}
          </Button>
          <Button
            size="small"
          ...
          {src && (
            <Button size="small" icon={<Download size={13} />} onClick={onDownload} className="text-xs">
      ...
      {error && <p className="px-3.5 pb-3 text-xs font-medium text-error">{error}</p>}
    </div>
  )
}
