...
const AVATAR_TONES: VoiceAvatarTone[] = [
  "purple",
  "green",
  "teal",
  "coral",
...
export function voiceInitials(label: string) {
  ...
  const parts = cleaned
    .split(/[\s-]+/)
    .filter((part) => part.length > 0 && !/^(male|female)$/i.test(part));
  if (parts.length >= 2) {
...
export function voiceMetaLine(voiceId: string, label: string) {
  ...
  const gender = /female/i.test(voiceId)
    ? "Female"
    : /male/i.test(voiceId)
      ? "Male"
      : "Voice";
...
export function recordRecentVoice(serverId: string, voiceId: string) {
  ...
  const next = [voiceId, ...current.filter((id) => id !== voiceId)].slice(
    0,
    MAX_RECENT_VOICES,
  );
  store[serverId] = next;
...
export function buildStylePrompt(emotion: string, style: string, base = "") {
  const parts = [
    ...
    style !== "narration" ? `${style} delivery` : "",
  ].filter(Boolean);
  return parts.join(". ");
}
