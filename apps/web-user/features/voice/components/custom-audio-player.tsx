...
function WaveformTrack({
  ...
  const visibleBars = useMemo(() => {
    ...
    return Array.from(
      { length: numVisible },
      (_, i) => bars[Math.round((i / numVisible) * bars.length)] ?? MIN_H,
    );
  }, [bars, numVisible]);
    ...
    <div ref={outerRef} className="relative min-w-0 flex-1">
      ...
      >
        {visibleBars.map((h, i) => {
          ...
          const nearScrub =
            Math.abs((i / visibleBars.length) * 100 - scrubPct) < 4;
          return (
            <div
              key={i}
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            boxShadow:
              "0 2px 8px color-mix(in srgb, var(--color-primary) 50%, transparent)",
          }}
        >
          {playing ? (
        ...
        >
          {formatTime(current)}
          {!isNarrow && " / "}
          {!isNarrow && formatTime(duration)}
          {isNarrow && (
            <span style={{ opacity: 0.45 }}> / {formatTime(duration)}</span>
          )}
        </span>
