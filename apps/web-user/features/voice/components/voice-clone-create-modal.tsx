...
export function VoiceCloneCreateModal({
  open,
  onClose,
  onSuccess,
}: VoiceCloneCreateModalProps) {
  const { t } = useLocale();
  ...
  const effectiveProvider = useMemo(() => {
    if (providerOptions.some((option) => option.value === provider))
      return provider;
    return (
      (providerOptions[0]?.value as "mimo" | "siliconflow" | undefined) ??
      provider
    );
  ...
  const handleSubmit = async () => {
    if (!name.trim()) {
      message.warning(t("voiceStudio.nameRequired"));
      return;
    }
    if (!audioFile) {
      message.warning(t("voiceStudio.audioRequired"));
      return;
    }
    if (!termsAccepted) {
      message.warning(t("voiceStudio.termsRequired"));
      return;
    }
    ...
    try {
      ...
      message.error(
        error instanceof Error ? error.message : t("voiceStudio.createFailed"),
      );
    }
  };

  const languageOptions = VOICE_CLONE_LANGUAGES.map((lang) => ({
    value: lang.value,
    label: lang.label,
  }));
  const genderOptions = [
      ...
      footer={
        <div className="flex flex-col gap-3 pb-2">
          ...
          <Button
            ...
            size="large"
            block
            loading={createClone.isPending}
            disabled={Boolean(activeProviderHint)}
      ...
      <div className="flex flex-col gap-5">
        {activeProviderHint && (
          <Alert
            type="warning"
            showIcon
            title={t("voiceStudio.apiKeyTitle")}
            description={activeProviderHint}
          />
        )}

        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-medium text-secondary">
            {t("voiceStudio.provider")}
          </label>
          <Select
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-medium text-secondary">
            {t("voiceStudio.voiceName")} <span className="text-red-500">*</span>
          </label>
          <Input
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder={t("voiceStudio.voiceNamePlaceholder")}
            maxLength={120}
            size="large"
          />
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-medium text-secondary">
            {t("voiceStudio.gender")} <span className="text-red-500">*</span>
          </label>
          <Segmented
            block
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-medium text-secondary">
            {t("voiceStudio.language")} <span className="text-red-500">*</span>
          </label>
          <Select
            value={language}
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <Button
            ...
            size="large"
            onClick={() => avatarInputRef.current?.click()}
            className="border-accent text-accent hover:border-accent/70 hover:text-accent/70"
          >
            {avatarPreview
              ? t("voiceStudio.avatarChange")
              : t("voiceStudio.avatarHint")}
          </Button>
        </div>

        <div className="flex flex-col gap-2">
          <label className="text-sm font-medium text-secondary">
            {t("voiceStudio.referenceAudio")}{" "}
          ...
          {!audioFile && (
            <Segmented
              ...
              options={[
                {
                  ...
                  label: (
                    <div className="flex items-center justify-center gap-1.5 py-0.5">
                      <UploadIcon size={14} />
                      <span>{t("voiceStudio.audioModeUpload")}</span>
                    </div>
                  ),
                },
                {
                  ...
                  label: (
                    <div className="flex items-center justify-center gap-1.5 py-0.5">
                      <Mic size={14} />
                      <span>{t("voiceStudio.audioModeRecord")}</span>
                    </div>
                  ),
                },
              ]}
              ...
              onChange={(v) => {
                ...
                setAudioInputMode(v as "upload" | "record");
              }}
            />
          )}
          ...
          {audioFile && audioObjectUrl ? (
            <div className="flex flex-col gap-2 rounded-xl border border-default bg-surface p-3">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-muted transition-colors hover:bg-red-500/10 hover:text-red-500"
            ...
            <div
              className={[
                ...
                createClone.isPending || Boolean(activeProviderHint)
                  ? "cursor-not-allowed border-default opacity-50"
                  : "border-default hover:border-accent/50 hover:bg-accent-muted/20",
              ].join(" ")}
              onClick={() => {
                if (!createClone.isPending && !activeProviderHint)
            ...
            >
              ...
              <div>
                <p className="text-sm font-semibold text-secondary">
                  {t("voiceStudio.uploadAudio")}
                </p>
                <p className="mt-0.5 text-xs text-muted">
                  {t("voiceStudio.uploadHint")}
                </p>
              </div>
            </div>
          ) : (
              ...
              {isRecording ? (
                <div className="flex flex-col items-center gap-3 px-6 py-5">
                  ...
                  >
                    {Array.from({ length: VISUALIZER_BARS }).map((_, i) => (
                      <div
                        key={i}
                        className="w-[5px] shrink-0 rounded-full bg-accent"
                        style={{
                          height: "4px",
                  ...
                  <button
                    ...
                    className="flex h-12 w-12 items-center justify-center rounded-full bg-red-500 text-white shadow-lg ring-4 ring-red-500/20 transition-transform hover:scale-105 active:scale-95"
                    aria-label={t("voiceStudio.recordStop")}
                  >
                    <Square size={16} fill="white" />
                  </button>
                  <p className="text-xs text-muted">
                    {t("voiceStudio.recordStop")}
                  </p>
                </div>
              ) : (
                <div className="flex flex-col items-center gap-3 px-6 py-6">
                  <button
                    ...
                    className="group relative flex h-[72px] w-[72px] items-center justify-center rounded-full bg-accent text-white shadow-lg ring-4 ring-accent/20 transition-all hover:scale-105 hover:shadow-xl hover:ring-accent/40 active:scale-95"
                    aria-label={t("voiceStudio.recordStart")}
                  >
                    <Mic size={28} />
                  </button>
                  <div className="text-center">
                    <p className="text-sm font-semibold text-secondary">
                      {t("voiceStudio.recordStart")}
                    </p>
                    <p className="mt-0.5 text-xs text-muted">
                      {t("voiceStudio.recordHint")}
                    </p>
                  </div>
                </div>
              )}
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-medium text-secondary">
            {t("voiceStudio.referenceTranscript")}
          </label>
          <Input.TextArea
            value={referenceText}
            onChange={(e) => setReferenceText(e.target.value)}
            rows={3}
            placeholder={t("voiceStudio.referenceTranscriptPlaceholder")}
          />
        </div>
      </div>
    </Drawer>
