...
export function useDeleteWebhook() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (id: string) => deleteApi<ApiResponse<{ ok: boolean }>>(`/api/v1/admin/webhooks/${id}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin', 'webhooks'] }),
  })
}
...
export function useCreateApiKey() {
  ...
  return useMutation({
    mutationFn: (body: { name: string; scopes: string[] }) =>
      postApi<ApiResponse<ApiKeyCreateResponse>>('/api/v1/admin/api-keys', body),
...
export function useDeleteApiKey() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (id: string) => deleteApi<ApiResponse<{ ok: boolean }>>(`/api/v1/admin/api-keys/${id}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin', 'api-keys'] }),
  })
}
...
export function useAdminFeatureFlags() {
  return useQuery({
    ...
    queryFn: () => getApi<ApiResponse<FeatureFlag[]>>('/api/v1/admin/feature-flags'),
  })
}
...
export function useCreateFeatureFlag() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (body: { key: string; description?: string; enabled: boolean; rollout: number }) =>
...
export function useUpdateFeatureFlag() {
  ...
  return useMutation({
    mutationFn: ({ id, body }: { id: string; body: Record<string, unknown> }) =>
      putApi<ApiResponse<FeatureFlag>>(`/api/v1/admin/feature-flags/${id}`, body),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin', 'feature-flags'] }),
  })
}
...
export function useDeleteFeatureFlag() {
  ...
  return useMutation({
    mutationFn: (id: string) =>
      deleteApi<ApiResponse<{ ok: boolean }>>(`/api/v1/admin/feature-flags/${id}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin', 'feature-flags'] }),
  })
}
...
export function useStripeSettings() {
  return useQuery({
    ...
    queryFn: () => getApi<ApiResponse<StripeSettings>>('/api/v1/admin/billing/stripe'),
  })
}
...
export function useUpdateStripeSettings() {
  ...
  return useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin', 'billing', 'stripe'] }),
  })
}
...
export function useDeleteBackup() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (id: string) => deleteApi<ApiResponse<{ ok: boolean }>>(`/api/v1/admin/backups/${id}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin', 'backups'] }),
  })
}
...
export function useDeleteJob() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (id: string) => deleteApi<ApiResponse<{ ok: boolean }>>(`/api/v1/admin/jobs/${id}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin', 'jobs'] }),
  })
}
...
export function useAdminNotifications() {
  return useQuery({
    ...
    queryFn: () => getApi<ApiResponse<NotificationTemplate[]>>('/api/v1/admin/notifications'),
  })
}
...
export function useCreateNotification() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (body: { name: string; channel: string; enabled: boolean; sent_30d?: number }) =>
      postApi<ApiResponse<NotificationTemplate>>('/api/v1/admin/notifications', body),
...
export function useUpdateNotification() {
  ...
  return useMutation({
    mutationFn: ({ id, body }: { id: string; body: Record<string, unknown> }) =>
      putApi<ApiResponse<NotificationTemplate>>(`/api/v1/admin/notifications/${id}`, body),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin', 'notifications'] }),
  })
}
...
export function useDeleteNotification() {
  ...
  return useMutation({
    mutationFn: (id: string) =>
      deleteApi<ApiResponse<{ ok: boolean }>>(`/api/v1/admin/notifications/${id}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin', 'notifications'] }),
  })
}
