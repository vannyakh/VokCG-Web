...
import type { ApiResponse, WorkspaceContext, WorkspacePlan } from '@vokcg/types'
...
export function useWorkspaceContext(tenantId?: string | null) {
  ...
  return useQuery({
    ...
    queryFn: () => getApi<ApiResponse<WorkspaceContext>>(`/api/v1/workspace/me${query}`),
    select: (res) => res.data,
...
export function useWorkspacePlans() {
  ...
  return useQuery({
    ...
    queryFn: () => getApi<ApiResponse<WorkspacePlan[]>>('/api/v1/workspace/plans'),
    select: (res) => res.data,
    enabled: Boolean(accessToken),
    staleTime: 60_000,
  })
...
export function useWorkspaceCheckout() {
  return useMutation({
    mutationFn: (body: CheckoutInput) =>
      postApi<ApiResponse<WorkspaceCheckoutResult>>('/api/v1/workspace/me/checkout', body),
  })
}
