...
export const useAdminUiStore = create<AdminUiState>()(
  persist(
    (set, get) => ({
      ...
      setTransitionProgressBar: (transitionProgressBar) => set({ transitionProgressBar }),
      ...
      setTransitionAnimation: (transitionAnimation) => set({ transitionAnimation }),
      ...
      openTab: (tab) =>
        set((state) => {
          ...
          return {
            ...
            tabHistory: inHistory ? state.tabHistory : [...state.tabHistory, tab],
          }
        }),
      ...
      closeOtherTabs: (keep) =>
        set((state) => {
          ...
          for (const tab of state.tabHistory) {
            if (tab !== keep && !ADMIN_AFFIX_TABS.includes(tab)) nextRemoved.add(tab)
          }
      ...
      sortTabs: (oldIndex, newIndex, activeTab) => {
        ...
        const state = get()
        const openTabs = selectOpenTabs(state.tabHistory, state.removedTabs, activeTab)
        const reordered = [...openTabs]
        const [moved] = reordered.splice(oldIndex, 1)
        if (!moved) return
        reordered.splice(newIndex, 0, moved)
        const closed = state.tabHistory.filter((t) => state.removedTabs.includes(t))
        set({
          tabHistory: [
            ...reordered,
