...
function SourceSection({ icon: Icon, title, copyText, copyLabel, copiedLabel, children }: SourceSectionProps) {
  return (
    <motion.div variants={fadeUpItem} className="border-b border-divider px-4 py-3.5 last:border-b-0">
      <div className="mb-2.5 flex items-center justify-between gap-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className="text-xs font-bold tracking-wide uppercase text-secondary">{title}</span>
        </div>
        {copyText ? <CopyIconButton text={copyText} label={copyLabel} copiedLabel={copiedLabel} size={12} /> : null}
      </div>
      {children}
    </motion.div>
  )
                    ...
                    {task.script && (
                      ...
                      >
                        <div className="relative overflow-hidden rounded-xl border border-divider bg-subtle/50 px-3.5 py-3 font-mono text-[11px] leading-relaxed text-secondary select-text">
                          ...
                          <p className="line-clamp-6 whitespace-pre-wrap pl-1.5">{task.script}</p>
                        </div>
                      </SourceSection>
                    )}
