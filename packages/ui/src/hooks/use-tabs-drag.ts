 ...
 */
export function useTabsDrag({ tabs, draggable = true, onSortTabs }: UseTabsDragOptions) {
    ...
    (index: number, tab: AdminTab) => {
      ...
      return {
        ...
        dragClassName: [
          ...
          dropIndex === index && dragIndex !== index ? 'ring-1 ring-[color-mix(in_srgb,var(--color-primary)_30%,transparent)]' : '',
        ]
          .filter(Boolean)
          .join(' '),
      }
