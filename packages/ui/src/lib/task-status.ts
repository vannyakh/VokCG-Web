...
export function getRenderStatus(task: Pick<Task, 'progress' | 'stage' | 'state'> | undefined): RenderStatus {
  if (!task) {
...
export function fitAspectBox(containerW: number, containerH: number, ratioW: number, ratioH: number) {
