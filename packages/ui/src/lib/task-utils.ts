...
export function filterTasks(tasks: Task[], searchQuery: string, statusFilter: TaskStatusFilter): Task[] {
  const query = searchQuery.trim().toLowerCase()

  return tasks.filter((task) => {
    if (statusFilter === 'processing' && !isTaskActive(task.state)) return false
